...
class Website extends CI_Controller {
 ...
 $this->send_msg($data['booksondemand_phone'], 'Thanks for Demand Book. We will Shortly contact you.');
    ...
    function signup() {
        ...
        if ($this->session->userdata("engguserid") == FALSE) {
            ...
            if ($this->input->post('submit')) {
                ...
                unset($data['check']);
                $pas=$data['user_password'];
                $data['user_password'] = md5($data['user_password']);
                $id23 = $this->login_model->get_record_where('user', array('user_email'=>$data['user_email']));
                 $id33 = $this->login_model->get_record_where('user', array('user_contact_no'=>$data['mobile_number']));
                 if(empty($id23) && empty($id33)){
                 
                $data['user_contact_no']=$data['mobile_number'];
 ...
 $this->send_msg($data['user_contact_no'], 'Thanks for registration on Disnip. Your login ID - '.$data['user_email'].'  and password - '.$pas.' ');
                 redirect('website');
                 
                 }else{
                  $this->session->set_flashdata('lerror', 'Email ID or Mobile Number already exist!');
    ...
    function login() {
        if ($this->session->userdata("engguserid") == FALSE) {
            ...
            if ($this->input->post('submit')) {
                ...
                $user_table = 'user';
                $where = array('user_email' => $this->input->post('user_email'), 'user_password' => md5($this->input->post('password')));
    ...
    function subcategories() {
        $cat_id = base64_decode($this->uri->segment(3));
         if ($cat_id=='10') {
            redirect('website/bookonmagazin');
        } elseif ($cat_id=='21') {
            redirect('website/kalyanipub');
        }
    ...
    function delivery_shipping() {
        ...
        $this->load->view('website/header');
        $this->load->view('website/delivery_shipping');
        $this->load->view('website/footer');
    }
    ...
    function privecy_policy() {
        ...
        $this->load->view('website/header');
        $this->load->view('website/privecy_policy');
        $this->load->view('website/footer');
    }
...
function mydashboard() 
{
    ...
    if (!empty(SESSIONKEY))
    {
         ...
         if ($this->input->post('submit')) {
                ...
                unset($data['user_id']);
                $user_update = $this->login_model->update_data('user', $data, $where);
                $this->session->set_flashdata('status', 'Profile Updated successfully');
                 redirect('website/mydashboard');
            } else {
                $where = array('user_id' => SESSIONKEY);
                $data['user'] = $this->login_model->get_column_data_where('user', '', $where);
                $this->load->view('website/header');
                $this->load->view('website/my_dashboard',$data);
                $this->load->view('website/footer');
            }
    }else{
        redirect('website');
...
function change_password()
{
    if (!empty(SESSIONKEY))
    {
         ...
         if ($this->input->post('submit')) {
                 $where = array('user_id' => $this->input->post('user_id'), 'user_password' => md5($this->input->post('old_pass')));
                 $check_password = $this->login_model->get_record_where('user', $where);
                 if ($check_password == FALSE) {
                     $this->session->set_flashdata('status', 'Old Password does not match');
                        redirect('website/mydashboard');
                 }else{
                      $data1 = array('user_password' => md5($this->input->post('new_pass')));
                      $password=$this->input->post('new_pass');
                       $email=$check_password[0]->user_email;
                       $where1 = array('user_id' =>  $this->input->post('user_id'));
                       $user_update = $this->login_model->update_data('user', $data1, $where1);
                        $this->send_msg($check_password[0]->user_contact_no, 'Welcome to Disnip. Your New password is - '.$password.' for Login Email is-'. $email);
               
                      $this->session->set_flashdata('status', 'New Password updated successfully');
                        redirect('website/mydashboard');
                 }
    ...
    function kalyanipub() {
//         `k_title`, `k_branch`, `k_u_name`, `k_u_email`, `k_u_contact`, `k_u_msg`
        ...
        if ($this->input->post('submit')) {
            ...
            $this->login_model->insert_data('kalyani', $data);
           $this->send_msg($data['k_u_contact'], 'Thanks for Demand of Kalyani Publication Books. We will Shortly contact you.');

            $subject = "Request For Kalyani Publication Books On Demand";
            $adminemail = $data1['contactus'][0]->company_email;
            $mail1 = 'Name:' . $data['k_u_name'] . " <br>";
            $mail1 .= 'Mobile:' . $data['k_u_contact'] . "<br>";
            $mail1 .= 'Email:' . $data['k_u_email'] . "<br>";
            $mail1 .= 'Book Name:' . $data['k_title'] . "<br>";
            $mail1 .= 'Book Branch:' . $data['k_branch'] . "<br>";
            $mail1 .= 'Message:' . $data['k_u_msg'];
            $headers1 = "MIME-Version: 1.0" . "\r\n";
            $headers1 .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
            $headers1 .= "From:" . $data['k_u_email'] . "\r\n" . "Reply-To: " . $data['k_u_email'] . "\r\n" . "X-Mailer: PHP/" . phpversion();
            $mail = mail($adminemail, $subject, $mail1, $headers1);

            $user_email = $data['k_u_email'];
            $subject3 = "Request For Kalyani Publication Books On Demand";
            $mail_msg3 .= 'Hello! ' . $data['k_u_name'] . "<br>";
            $mail_msg3 .= 'Thanks For Contact to Disnip!' . "<br>";
            $mail_msg3 .= "We Will shortly connect you!." . "<br>";
            $mail_msg3 .= 'Book Name:' . $data['k_title'] . "<br>";
            $mail_msg3 .= 'Book Branch:' . $data['k_branch'] . "<br>";
            $mail_msg3 .= "Request Massage: " . $data['k_u_msg'] . "<br>";
            ...
            $this->load->view('website/header');
            $this->load->view('website/kalyani_publication');
            $this->load->view('website/footer');
        }
    }

    function bookonmagazin() {
        // `m_id`, `m_title`, `m_name`, `m_contact`, `m_email`, `m_msg`, `m_sub`
        $where = array('contact_us_status' => 1);
        $data1['contactus'] = $this->login_model->get_data_where_condition('contact_us', $where);
        if ($this->input->post('submit')) {
            ...
            $this->login_model->insert_data('magzine', $data);

             $this->send_msg($data['m_contact'], 'Thanks for Demand of Magazine. We will Shortly contact you.');
            $subject = "Request For Magazine";
            $adminemail = $data1['contactus'][0]->company_email;
            $mail1 = 'Name:' . $data['m_name'] . "<br>";
            $mail1 .= 'Mobile:' . $data['m_contact'] . "<br>";
            $mail1 .= 'Email:' . $data['m_email'] . "<br>";
            $mail1 .= 'Magazine Name:' . $data['m_title'] . "<br>";
            $mail1 .= 'Subscription:' . $data['m_sub'] . "<br>";
            $mail1 .= 'Message:' . $data['m_msg'];
            $headers1 = "MIME-Version: 1.0" . "\r\n";
            $headers1 .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
            $headers1 .= "From:" . $data['m_email'] . "\r\n" . "Reply-To: " . $data['m_email'] . "\r\n" . "X-Mailer: PHP/" . phpversion();
            $mail = mail($adminemail, $subject, $mail1, $headers1);

            $user_email = $data['m_email'];
            $subject3 = "Request For Magazine";
            $mail_msg3 .= 'Hello! ' . $data['m_name'] . "<br>";
            $mail_msg3 .= 'Thanks For Contact to Disnip!' . "<br>";
            $mail_msg3 .= "We Will shortly connect you!." . "<br>";
            $mail_msg3 .= "Request for Magazine: <br>";
            $mail_msg3 .= 'Magazine Name:' . $data['m_title'] . "<br>";
            $mail_msg3 .= 'Subscription:' . $data['m_sub'] . "<br>";
            $mail_msg3 .= 'Message:' . $data['m_msg'];
            $headers = "MIME-Version: 1.0" . "\r\n";
            $headers .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
            $headers .= "From: Disnip" . $data1['contactus'][0]->company_email . "\r\n" . "Reply-To: " . $data1['contactus'][0]->company_email . "\r\n" . "X-Mailer: PHP/" . phpversion();
            // $headers .= 'Cc: myboss@example.com' . "\r\n";
            ...
            $this->load->view('website/header');
            $this->load->view('website/book_on_magazine');
            $this->load->view('website/footer');
        }
    }
    ...
    function forgot_pass() {
        if ($this->input->post('submit')) {
            ...
            $where1 = array('user_contact_no' => $data['user_email']);
            ...
            if (!empty($datause)) {
                ...
                //echo $datause[0]->user_contact_no;
                
                $this->send_msg($datause[0]->user_contact_no, 'Welcome to Disnip. Your New password is - '.$pass.' for Login Email is-'. $datause[0]->user_email);
                $subject = "Request New Password";
                $adminemail = $datause[0]->user_email;
...
$words= str_replace(' ', ' +', $data['search']);
//foreach ($search as $value) {
//   if(empty($keywords)){
//    $keywords="`product_name` LIKE '%" . $value . "%' OR `category_name` LIKE '%" . $value . "%'  OR `subcategory_name` LIKE '%" . $value . "%'  OR `product_author` LIKE '%" . $value . "%'  OR `product_publication` LIKE '%" . $value . "%'  OR `product_edition` LIKE '%" . $value . "%'"  ; 
//   }else{
//       $keywords=$keywords." OR `product_name` LIKE '%" . $value . "%' OR `category_name` LIKE '%" . $value . "%'  OR `subcategory_name` LIKE '%" . $value . "%'  OR `product_author` LIKE '%" . $value . "%'  OR `product_publication` LIKE '%" . $value . "%'  OR `product_edition` LIKE '%" . $value . "%'";
//   } 
//}

        $data1['product'] = $this->login_model->get_query_result("SELECT * from `product` inner join category on product_category_id=category_id inner join subcategory on product_subcategory_id=subcategory_id where   MATCH (product_name, product_author,product_publication,product_edition ) AGAINST ('+".$words."'  ) OR MATCH (product_name, product_author,product_publication,product_edition ) AGAINST ('+".$words."' IN BOOLEAN MODE ) group by product_id ");
        $this->load->view('website/header');
    ...
    function get_order() {
        ...
        $where_contact_us = array('contact_us_id' => '1');
        $contact_us = $this->login_model->get_column_data_where('contact_us', '', $where_contact_us);
        if (!empty($address)) {
            if (!empty($cart)) {
                ...
                $user_mobile_no = $address[0]->user_mobile_no;
                ...
                foreach ($cart as $value) {
                    ...
                    $qty_update = "UPDATE product SET product_of_stock=product_of_stock-" . $value->cart_product_qty . " where product_id =" . $value->cart_product_id;

                    $stck_update = mysql_query($qty_update);
                }
...
<table cellpadding="0" cellspacing="0" width="600" style="background:#fff; border:1px solid #cbcbcb; margin:0 auto; font-family:Arial, Helvetica, sans-serif; font-size:12px;">
        	...
        	<td>
            	<table style="margin:0 auto; border:1px solid #DDD; width:100%" border="0" cellpadding="0" cellspacing="0">
                	<thead>
                    	<tr>
                            ...
                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;  background:#4D0684; color:#fff;"><strong>Product ID</strong></td>
                ...
                while ($row = mysql_fetch_array($response_order_rec)) {
                    ...
                    $mail_msg3 .= '<tr>
                            ...
                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;">' . $row['p_id'] . '</td>
                            ...
                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;"><i class="fa fa-rupee">Rs </i><span>' . $total . '</span></td>
                        </tr>';
                    ...
                    <tfoot>
                    	<tr>
                            ...
                            <td colspan="3" style="border-bottom:1px solid #ddd; padding:10px;">    
                            	<p style="text-align:right;"><strong>Sub Total:</strong><i class="fa fa-rupee">Rs </i><span>' . $grandtotal . '</span></p>
                                <p style="text-align:right;"><strong>Shipping (10Rs Include GST):</strong> <i class="fa fa-rupee">Rs </i>' . $delivery_charge . '</p>
                                <P style="text-align:right;"> <strong>Extra Charge:</strong> <i class="fa fa-rupee">Rs </i>00</P>
                            </td>
                        </tr>
                        <tr style="background:#eee;">
                        	<td align="right" colspan="5" style="border-bottom:1px solid #ddd; padding:10px;"><strong>Grand Total: <i class="fa fa-rupee">Rs </i><span>' . $main_total . '</span></strong></td>
                        </tr>
                    </tfoot>
                </table>
            </td>
    ...
    <tfoot style="background:#4D0684; text-align:center; color:#fff;">
        <tr>
        	<td style="color:#fff;"><p>Copyright 2017 Disnip | All Rights Reserved</p></td>
        <tr>
    </tfoot>
                ...
                $headers = "MIME-Version: 1.0" . "\r\n";
                $headers .= "Content-type: text/html; charset=iso-8859-1" . "\r\n";
                $headers .= "From:" . $contact_us[0]->company_email . "\r\n" . "Reply-To:" . $contact_us[0]->company_email . "\r\n";
                $headers .= "Cc:" . $contact_us[0]->company_email . "\r\n" . "X-Mailer: PHP/" . phpversion();
                $mail = mail($user_email, $subject, $mail_msg3, $headers);
                if ($order) {
                 //   $this->send_msg($user_mobile_no, 'Thanks for order from disNip.com. Your Order amount is-'.$main_total.' With Order ID is-'.$cid);
                 $mob=$user_mobile_no.",".$contact_us[0]->contact_no;
                    $this->send_msg($mob, 'Hello '.$user_name.'. Your order has been recived for Order ID: ' . $order . ' from of Order amount is-'.$main_total.'Rs With Transction No is-'.$cid);
...
//             $where = array('contact_us_status' => 1);
//             $data1['contactus'] = $this->login_model->get_data_where_condition('contact_us', $where);
//            $company_email=$data1['contactus'][0]->company_email;
//		 //$path1='http://72.167.41.165/violet/images/logo.png';
//		 $path1=  websitelogo; 
//			//	$path1='http://'.$_SERVER['HTTP_HOST'].'/violet/images/logo.png';
//			$subject = 'Disnip order status';
...
//                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;">' . $row['order_quantity'] . '</td>
//                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;">' . $row['order_unit_price'] . '</td>
//                            <td style="border-right:1px solid #DDD; border-bottom:1px solid #ddd; padding:10px;">Rs <span>' . $total . '</span></td>
//                        </tr>';
//					$grandtotal=$grandtotal+$total;
//			}
...
//                            </td>
//                            <td colspan="3" style="border-bottom:1px solid #ddd; padding:10px;">    
//                            	<p style="text-align:right;"><strong>Sub Total:</strong>Rs <span>' . $grandtotal . '</span></p>
//                                <p style="text-align:right;"><strong>Shipping:</strong> Rs '.$delivery_charge.'</p>
//                                <P style="text-align:right;"> <strong>Extra Charge:</strong> Rs 00</P>
//                            </td>
//                        </tr>
//                        <tr style="background:#eee;">
//                        	<td align="right" colspan="5" style="border-bottom:1px solid #ddd; padding:10px;"><strong>Grand Total: Rs <span>' . $main_total . '</span></strong></td>
//                        </tr>
//                    </tfoot>
//                </table>
//            </td>
...
//    <tfoot style="background:#4D0684; text-align:center; color:#fff;">
//        <tr>
//        	<td style="color:#fff;"><p>Copyright Â© 2017 Disnip.com</p></td>
//        <tr>
//    </tfoot>
//    
//</table>
...
//return $rand;
    ...
    function loginutility_sms() {
        $api_key = '3592FF67589786';
        $contacts = '9770543137';
        $from = 'RAHULD';
