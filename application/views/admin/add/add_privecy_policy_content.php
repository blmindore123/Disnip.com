    ...
    <div class="panel-body">
        <form <?php if(!empty($privecy_policy)){ ?>action="<?php echo site_url('admin/edit_privecy_policy_content'); ?>"<?php } else { ?>action="<?php echo site_url('admin/add_about_content'); ?>"<?php } ?> role="form" id="form1" method="post" class="validate" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <input type="text" name="title" <?php if(!empty($privecy_policy)){ ?>value="<?php echo $privecy_policy[0]->title; } ?>" class="form-control" data-validate="required" placeholder="Content title" />
            </div>
          <div class="form-group">
                <label class="control-label">Content</label>
             <textarea class="form-control ckeditor" rows="10" name="privecy_policy_content"  data-validate="required"  placeholder=" Content "><?php if(!empty($privecy_policy)){ ?> <?php echo $privecy_policy[0]->privecy_policy_content ; } ?></textarea>
            </div>
            <div class="form-group">
