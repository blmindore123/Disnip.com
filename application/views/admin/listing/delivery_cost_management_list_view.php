...
<!--      <div class="panel-options">
    ...
    <div id="user_list" class="panel-body">
        ...
        <table id="example-1" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			<tbody>
				<?php
                  if (!empty($delivery_cost_management_list)) {
                    $n = 1;
                    foreach ($delivery_cost_management_list as $value) {
                        ?>
                        <tr>
                            <td style="width: 10%"><?php echo $n; ?></td>
                            <td><?php if($value->delivery_cost_type==2){ echo "Paid"; }else{ echo "Free"; } ?></td>
                            <td><?php if($value->delivery_cost_type==2){ echo $value->delivery_price; }else{ echo "None"; } ?></td>
                            <td style="width: 25%">
                               
                                <?php $status = $value->delivery_cost_status; ?>
                            
                        <a href="<?php echo site_url('admin/edit_delivery_cost_management').'/'.$value->delivery_cost_id; ?>" ><i class="fa-pencil icon-blue"></i>
                                                       &nbsp;
                                <?php if ($status == 1) { ?>
