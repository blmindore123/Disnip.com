    ...
    <div id="user_list" class="panel-body">
        ...
        <table id="example-1" class="table table-striped table-bordered" cellspacing="0" width="100%">
            <thead>
                <tr>
                    <th>S. No.</th>
                    <th>Minimum Checkout Cost</th>
                    <th>Action</th>
                </tr>
            </thead>
			<tbody>
				<?php
                  if (!empty($minimum_checkout_list)) {
                    $n = 1;
                    foreach ($minimum_checkout_list as $value) {
                        ?>
                        <tr>
                            <td style="width: 10%"><?php echo $n; ?></td>
                            <td><?php echo $value->minimum_checkout_cost; ?></td>
                           
                            <td style="width: 25%">
                               
                                <?php $status = $value->minimum_checkout_status; ?>
                            
                        <a href="<?php echo site_url('admin/edit_minimum_checkout').'/'.$value->minimum_checkout_id; ?>" ><i class="fa-pencil icon-blue"></i>
                                                       &nbsp;
                                <?php if ($status == 1) { ?>
